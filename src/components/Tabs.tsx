        ...
        <TabsPrimitive.Root defaultValue="tab1">
            <TabsPrimitive.List className={cx('flex w-full rounded-t-lg bg-white dark:bg-gray-800')}>
                {tabs.map(({ title, value }) => (
                    ...
                    >
                        <span className={cx('text-sm font-medium', 'text-gray-700 dark:text-gray-100')}>{title}</span>
                    </TabsPrimitive.Trigger>
                ))}
            </TabsPrimitive.List>
            {tabs.map(({ value }) => (
                <TabsPrimitive.Content key={`tab-content-${value}`} value={value} className={cx('rounded-b-lg bg-white px-6 py-4 dark:bg-gray-800')}></TabsPrimitive.Content>
            ))}
        </TabsPrimitive.Root>
    );
};
