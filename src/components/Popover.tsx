        ...
        <div className="relative inline-block text-left">
            <PopoverPrimitive.Root>
                ...
                <PopoverPrimitive.Content
                    align="center"
                    sideOffset={4}
                    className={cx('radix-side-top:animate-slide-up radix-side-bottom:animate-slide-down', 'w-48 rounded-lg p-4 shadow-md md:w-56', 'bg-white dark:bg-gray-800')}
                >
                        ...
                        {items.map(({ id, label, defaultValue }) => {
                            return (
                                <fieldset key={`popover-items-${id}`} className="flex items-center">
                                    ...
                                    <label htmlFor={id} className="shrink-0 grow text-xs font-medium text-gray-700 dark:text-gray-400">
                                        {label}
                                    </label>
                                    <input
                                        id={id}
