...
const DemoCard = ({ variant = Variant.Default, children, data: { title, link } }: Props) => {
    const id = title.replace(' ', '_').toLowerCase();

    return (
        <section id={id} className="h-[550px] w-full scroll-mt-6 overflow-hidden rounded-xl shadow">
            <div
                className={cx(
            ...
            >
                ...
                <div className="absolute inset-x-0 top-0 flex items-center justify-between bg-black/50 px-4 py-2.5 dark:bg-black/30">
                    <a href={`#${id}`} className="dark:ext-gray-300 select-none text-sm font-medium text-white">
                        {title}
                    </a>
                    <a href={link} rel={'noreferrer'} target={'_blank'} className="select-none rounded bg-white/25 px-2 py-1.5 text-xs font-medium text-white hover:bg-white/30 dark:text-gray-100">
                        Code
                    </a>
                </div>
            </div>
