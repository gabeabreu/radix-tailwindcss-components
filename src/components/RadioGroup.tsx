...
const RadioGroup = ({ items }: RadioGroupItems) => {
    return (
        <form>
            <fieldset>
                <legend className="text-sm font-medium leading-4 text-gray-900 dark:text-gray-100">Choose your starter</legend>
                <div className="mt-3 space-y-3">
                    {items.map((item) => (
                        <div key={item.id} className="flex items-center">
                            ...
                            />
                            <label htmlFor={item.id} className="ml-2 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                {item.title}
                            </label>
                        </div>
                    ))}
